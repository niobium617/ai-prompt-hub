...
export class FavoriteService {
  ...
  async add(userId: number, targetType: string, targetId: number) {
    const existing = await this.prisma.favorite.findUnique({
      where: { userId_targetType_targetId: { userId: BigInt(userId), targetType, targetId: BigInt(targetId) } },
    });
    if (existing) throw new ConflictException('已收藏');

    const fav = await this.prisma.favorite.create({
      data: { userId: BigInt(userId), targetType, targetId: BigInt(targetId) },
    });

    // 同步更新收藏数
    if (targetType === 'prompt') {
      await this.prisma.prompt.update({ where: { id: BigInt(targetId) }, data: { favoriteCount: { increment: 1 } } });
    }
  ...
  async remove(userId: number, targetType: string, targetId: number) {
    ...
    if (targetType === 'prompt') {
      await this.prisma.prompt.update({ where: { id: BigInt(targetId) }, data: { favoriteCount: { decrement: 1 } } });
    }
    return { success: true };
  }
}
